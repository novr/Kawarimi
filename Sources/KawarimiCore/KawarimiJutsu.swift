...
public enum KawarimiJutsu {
    ...
    public static func generateKawarimiHandlerSource(
        ...
        let methods = try generateHandlerMethods(document: document, namingStrategy: namingStrategy)
        return """
            // Generated by Kawarimi. Do not edit.
    ...
    private static func generateHandlerMethods(document: OpenAPI.Document, namingStrategy: KawarimiNamingStrategy) throws -> String {
        var lines: [String] = []
        let components = document.components
        ...
        for route in document.routes {
            for endpoint in route.pathItem.endpoints {
                ...
                let stub = try selectHandlerStubResponse(
                    operation: operation,
                    components: components,
                    operationId: operationId
                )
                lines.append("    package func \(methodName)(_ input: Operations.\(typeName).Input) async throws -> Operations.\(typeName).Output {")
                switch stub {
                case .okJSON(let expr):
                    lines.append("        return .ok(.init(body: .json(\(expr))))")
                case .okEmpty:
                    lines.append("        return .ok(.init())")
                case .createdJSON(let expr):
                    lines.append("        return .created(.init(body: .json(\(expr))))")
                case .createdEmpty:
                    lines.append("        return .created(.init())")
                case .noContent:
    ...
    /// Picks a handler stub aligned with swift-openapi-generator: JSON 200/201, empty-body 200/201 (no `content`), or 204 only.
    private static func selectHandlerStubResponse(
        ...
        for code in [200, 201] {
            ...
            if response.content[.json] != nil {
                guard let expr = try bodyExprForResponse(
                    ...
                    throw KawarimiJutsuError.handlerGenerationUnsupported(
                        operationId: operationId,
                        detail:
                            "HTTP \(code) に application/json がありますが schema を解決できません。スタブを生成できません。schema を追加するか、ボディなしの成功応答なら `content` を省略してください。"
                    )
                }
            ...
            if response.content.isEmpty {
                if let headers = response.headers, !headers.isEmpty {
                    throw KawarimiJutsuError.handlerGenerationUnsupported(
                        operationId: operationId,
                        detail:
                            "HTTP \(code) にレスポンスヘッダー定義がありますがボディがありません。スタブの初期化式を機械生成できないため、ハンドラを手実装するかヘッダーを見直してください。"
                    )
                }
            ...
            throw KawarimiJutsuError.handlerGenerationUnsupported(
                operationId: operationId,
                detail:
                    "HTTP \(code) に application/json 以外の content が含まれています。Kawarimi は JSON ボディまたは content なしの 200/201 のみスタブ化します。"
            )
        }
        ...
        throw KawarimiJutsuError.handlerGenerationUnsupported(
            operationId: operationId,
            detail:
                "HTTP 200 / 201（JSON または content なし）または 204 が必要です。swift-openapi-generator の Operations 出力と整合するスタブのみ生成します。"
        )
    }
    ...
    private static func bodyExprForResponse(
        ...
        return try swiftInitializerForSchema(schema, components: components, operationId: operationId)
    }
    ...
    private static func swiftInitializerForSchema(
        ...
        case .all, .one, .any, .not:
            throw KawarimiJutsuError.handlerGenerationUnsupported(
                operationId: operationId,
                detail:
                    "allOf / oneOf / anyOf / not の JSON schema はスタブ初期化を生成できません。スキーマを単純化するか、KawarimiHandler を手実装してください。"
            )
        ...
        if let allowed = resolved.allowedValues, !allowed.isEmpty {
            throw KawarimiJutsuError.handlerGenerationUnsupported(
                operationId: operationId,
                detail:
                    "enum（allowedValues）を含む schema は swift-openapi-generator が RawRepresentable 等を生成するため、文字列リテラルスタブは生成しません。列挙を避けるかハンドラを手実装してください。"
            )
        }

        if let objectContext = resolved.objectContext {
            let args = try objectContext.properties.map { name, propSchema -> String in
                let valueExpr = try swiftInitializerForSchema(propSchema, components: components, operationId: operationId)
