...
/// URLSession-backed upstream transport for Proxy forward (`URLRequest` in, `HTTPBody` out).
///
/// Use ``live()`` for production forwarding. Use ``mock(_:)`` in tests.
/// Custom `URLSession` / delegate injection is intentionally unsupported.
...
private enum KawarimiProxyStreamingURLSessionTransport {
    ...
    private static func send(
        ...
        if Self.isNoBodyStatus(http.statusCode) {
            try await Self.drain(asyncBytes)
            return (http, nil)
        }
        let responseBody = Self.responseBody(from: asyncBytes)
    ...
    private static func responseBody(from asyncBytes: URLSession.AsyncBytes) -> HTTPBody {
        let maxBytes = KawarimiProxyForwardLimits.maxResponseBodyBytes
        let (stream, continuation) = AsyncThrowingStream<ArraySlice<UInt8>, Error>.makeStream()
        Task {
            do {
                var total = 0
                var buffer = [UInt8]()
                buffer.reserveCapacity(responseChunkSize)
                for try await byte in asyncBytes {
                    buffer.append(byte)
                    if buffer.count >= responseChunkSize {
                        total += buffer.count
                        if total > maxBytes {
