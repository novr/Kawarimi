...
struct KawarimiPlugin: BuildToolPlugin {
    func createBuildCommands(context: PluginContext, target: Target) async throws -> [Command] {
        ...
        let outputOptions: KawarimiGeneratorOutputOptions
        do {
            outputOptions = try KawarimiGeneratorConfigLineParser.load(kawarimiConfigURL: kawarimiConfigURL)
        } catch let error as KawarimiGeneratorConfigLineParserError {
            switch error {
            case .allOutputsDisabled(let message):
                throw KawarimiPluginError.fileErrors([message])
            }
        }
        let outputNames = outputOptions.outputFileNames
