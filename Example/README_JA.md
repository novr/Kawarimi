...
任意の **dev Proxy sidecar**: **DemoServer** に **`KAWARIMI_UPSTREAM_URL`**（origin のみ。例 `https://staging.example.com`）を設定すると、オーバーライド未マッチが dev/staging API へ届く。未設定時は OpenAPI スタブのまま。 [Proxy（dev sidecar）](../docs/ja/henge.md#proxy-upstream-forward) を参照。
