...
**`SpecEndpointProviding`** 利用時はアップグレード後に **`KawarimiSpec.swift` を再生成**。エンドポイントの **`tags`** と **`parameters`** は OpenAPI どおり（無いとき `nil`）。parameters は path-item と operation をマージ（operation 優先）。cookie と content 形式の parameter は生成 spec に含めません。
