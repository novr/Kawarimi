...
**`SpecEndpointProviding`** 利用時はアップグレード後に **`KawarimiSpec.swift` を再生成**。エンドポイントの **`tags`**、**`parameters`**、**`requestBodies`** は OpenAPI どおり（無いとき `nil`）。parameters は path-item と operation をマージ（operation 優先）。cookie と content 形式の parameter は生成 spec に含めません。**`requestBodies`** は **`application/json`** のみ（named `examples` があれば全件、なければ合成デフォルト 1 行）。multipart・未解決 `$ref`・非 JSON は含めません。
