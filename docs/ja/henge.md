...
- **`KawarimiAdminRoute`** — `spec` / `status` / `configure` / `remove` / `reset` / `reload`。各 case に **`httpMethod`**・**`relativePath`**・**`successStatusCode`**（reload のみ `204`）。
...
admin ルートを Vapor に載せ、生成 handler 登録時に **`KawarimiServerMiddleware`** を渡す例:
...
- モックが当たったら **`next` を呼ばず** 合成レスポンスを返す。当たらなければ handler に委譲する。
