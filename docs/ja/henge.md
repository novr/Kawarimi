...
生成される型 **`SpecResponse`** は **`KawarimiFetchedSpec`** に準拠しており、**`KawarimiConfigView(client:specType:)`** が `/__kawarimi/spec` を手動クロージャなしでデコードできます。
...
**KawarimiHenge（`KawarimiConfigView`）:** API の `baseURL` に揃えた **`KawarimiAPIClient`** と、生成された **`SpecResponse.self`**（**`KawarimiFetchedSpec`** 準拠）を渡します。画面上のサーバー表記は **`client.baseURL.absoluteString`** です。マイナス（**Del**）は、モックがオンのとき **`isEnabled: false` を保存**します。すでにオフで、かつそのチップ用の**保存済み行**があるときは **`remove`** を呼び、サーバー設定から行を消してエディタを Spec のドラフトに戻します（HTTP は **KawarimiCore** の **`KawarimiAPIClient.removeOverride(override:)`** と同じ経路です）。
