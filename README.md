...
Default stub bodies prefer a literal `.json(...)` expression; when that cannot be generated from the schema, Kawarimi falls back to **`JSONDecoder`** on the same synthesized JSON as the `Kawarimi` transport mock ([Mock JSON rules](docs/mock-json.md#kawarimihandler-default-stubs)). Configure **`handlerStubPolicy`** in `kawarimi-generator-config.yaml` ([Integration](docs/integration.md)).
