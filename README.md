...
**Implicit dependency on OpenAPIGenerator**: The generated Kawarimi code is designed to work with the Types, Client, and Server produced by the official plugin (OpenAPIGenerator). Use both plugins on the same target. Compatible with **swift-openapi-generator 1.0.0 or later**.
...
- Include Command 1’s `outputFiles` (e.g. Types.swift, Client.swift, Server.swift) in Command 2’s `inputFiles`. SwiftPM’s build graph will run Command 2 after Command 1.
